...
export default {
    ...
    effects: {
        ...
        *serachRolesByKeyword({ payload }, { call, put }) {
        ...
        // 根据角色ID删除
        *deleteRoleByID({ payload }, { call, put }) {
            const newPayload = { id: payload, token: getToken() };
            const response = yield call(apiDeleteRoleByID, newPayload);
            if (response.code === 0) {
                yield put({
                    type: 'saveDeletedRoleByID',
                    payload: payload,
                 });
        ...
        *addRole({ payload }, { call, put }) {
        ...
        //查询角色
        *searchRoleByID({payload},{call,put}){
            const newPayload = {id:payload, token: getToken()};
            const response = yield call(apiSelectRoleByID,newPayload);
            if(response.code === 0){
                yield put({type: 'saveRoldInfo', payload: response, });
            }
            else{
                message.error(response.date);
            }
    ...
    reducers: {
        ...
        saveDeletedRoleById(state, { payload }) {
            return {
                ...state,
